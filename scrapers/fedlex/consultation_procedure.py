...
class ConsultationProcedure(dj_scrape.core.CouchDBMixin, dj_scrape.core.Scraper):
    ROOT_URL = "https://www.fedlex.admin.ch/"
    ENDPOINT = "https://www.fedlex.admin.ch/elasticsearch/proxy/_search?index=data"
    DB_NAME = "fedlex_consultation_procedure"
    SIZE = 10
    OFFSET = 0
            ...
            "query": {
                "bool": {
                    "must": [
                        ...
                        {"exists": {"field": "data.references.draftHasTask.keyword"}},
                        {
                            "exists": {
                                "field": "included.attributes.eventTitle.rdf:langString.fr"
                            }
                        },
                    ],
                    "filter": [{"terms": {"data.type.keyword": ["InitialDraft"]}}],
                }
            },
        }
